...
dsc_scores_raw <- read_csv("dqa_scores.csv")
...
order_layers <- c(order_layers_df$goa_lay)
...
dsc_layers <- dsc_scores_goal %>% 
  ...
  mutate(score = as.numeric(score), #, na.rm = TRUE
         ...
         goa_lay = as.character(goa_lay),
         criteria = factor(criteria, levels = order_criteria),
         goa_lay = factor(goa_lay, levels = order_layers)) 


### Heatmap!
dsc_goal_layers <- ggplot(data = dsc_layers, aes(x = criteria, y = goa_lay)) +
...
ggsave("figs/dsc_heatmap_paper_option2.jpg", width=10, height=8, dpi=300)
