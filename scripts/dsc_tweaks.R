...
order_layers_df <- dsc_scores_goal %>% 
  select(Go_lay,`Average Score` ) %>%
  unique()  %>% 
  arrange(`Average Score`)

order_layers <- c(order_layers_df$Go_lay)
...
dsc_layers <- dsc_scores_goal %>% 
  ...
  mutate(score = as.numeric(score), #, na.rm = TRUE
         ...
         Go_lay = as.character(Go_lay),
         criteria = factor(criteria, levels = order_criteria),
         Go_lay = factor(Go_lay, levels = order_layers)) 

### Heatmap!
dsc_goal_layers <- ggplot(data = dsc_layers, aes(x = criteria, y = Go_lay)) +
...
ggsave("figs/dsc_heatmap_paper_option2.5.jpg", width=10, height=8, dpi=300)
